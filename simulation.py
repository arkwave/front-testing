...
def apply_signal(pf, vdf, pdf, signals, date, next_date, roll_cond, strat='dist'):
    ...
    cols = ['delta_call_25_a', 'delta_put_25_b',
            'signal_diff', 'opmth', 'ftmth', 'pdt', 'lots', 'vega']
...
def liquidate_skew_pos(vega_req, num_skews, dval, calls, puts, pf, strat):
    ...
    Does so by selecting  a skew position to liquidate based on strat; currently implemented strategies for liquidation are 'dist' (the options selected are furthest away from dval) and 'filo' (first in last out), computing the vega per lot of that skew position, and liquidating the required number of lots. If the number of lots contained in the selected skew position is smaller than the number of lots required, the skew position is completely closed out (i.e. shorts are bought back, longs are sold) and the leftover vega is liquidated using the next highest skew pos, selected according to strat.

    If all options of a given type have been spent in this manner and liquidation is still not complete, the algorithm simply skips to handle options of the next type. In the pathological case where the entire portfolio's contents are insufficient to meet the liquidation condition, the entire portfolio is closed, and remaining deltas from hedging are closed off. 

    In practice, since we're interested in the call-side (long skew), lots required for the requisite vega is determined by the call option. Put option is instantiated with exactly the same number of lots, which could lead to a slight vega differential (currently being ignored, but might need to be addressed in the future).
    ...
    toberemoved = []
    resid_vega = vega_req * num_skews
    call_vega, put_vega = resid_vega, resid_vega
    ...
    while put_vega > 0:
        ...
        if strat == 'dist':
            ...
            put_ops = sorted(puts, key=lambda x: abs(
                abs(x.delta/x.lots) - dval))
            # print('putops: ', [str(p) for p in put_ops])
            if put_ops:
                max_put_op = put_ops[-1]

                print('putop selected: ', max_put_op)
                # handle puts
                p_vpl = abs(max_put_op.vega/max_put_op.lots)
                print('puts vega per lot: ', p_vpl)
                put_lots_req = round(abs(put_vega / p_vpl))
                ...
                # Case 2: lots required > lots available.
                    ...
                    put_vega -= max_put_op.lots * p_vpl
                    toberemoved.append(max_put_op)
                    puts.remove(max_put_op)
            else:
                print(
    ...
    while call_vega > 0:
        ...
        if strat == 'dist':
            ...
            call_ops = sorted(calls, key=lambda x: abs(
                abs(x.delta/x.lots) - dval))
            if call_ops:
                # print('callops: ', [str(p) for p in call_ops])
                max_call_op = call_ops[-1]
                print('callop selected: ', max_call_op)
                # handle calls
                c_vpl = abs(max_call_op.vega/max_call_op.lots)
                print('vega per lot: ', c_vpl)
                call_lots_req = round(abs(call_vega / c_vpl))
                print('call lots req: ', call_lots_req)

                # Case 1: call_lots_req < lots available
                if call_lots_req < max_call_op.lots:
                    print('l_calls: lots available.')
                    call_vega = 0
                    newlots = max_call_op.lots - call_lots_req
                    max_call_op.update_lots(newlots)
                    print('calls - new lots: ', newlots)
                    # break
                # Case 2: lots required >= lots available.
                else:
                    print('l_calls: lots unavailable.')
                    call_vega -= max_call_op.lots * c_vpl
                    toberemoved.append(max_call_op)
                    calls.remove(max_call_op)
            else:
                print('cannot liquidate positions any further. exiting...')
                break
        elif strat == 'filo':
            pass

    ...
    for i in range(len(chars)):
        ...
        try:
            d, g, t, v = _euro_barrier_euro_greeks(
                char, tau, vol, k, s, 0, payoff, direc, 'C', ki, ko, lots, barrier_vol=bvol, barrier_vol2=bvol)
        except TypeError:
            print(_euro_barrier_euro_greeks(
                char, tau, vol, k, s, 0, payoff, direc, 'C', ki, ko, lots, barrier_vol=bvol, barrier_vol2=bvol) is None)
...
def test_euro_barrier_pricing():
    ...
    for i in range(len(actuals)):
        ...
        val = _barrier_euro(char, tau, vol, k, s, 0,
                            payoff, direction, ki, ko, product, barrier_vol=vol, barrier_vol2=vol)
        try:
            assert np.isclose(val, actual)
        except AssertionError:

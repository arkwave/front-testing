    ...
    for i in range(len(chars)):
        ...
        try:
            d, g, t, v = _euro_barrier_euro_greeks(
                char, tau, vol, k, s, 0, payoff, direc, 'C', ki, ko, lots, bvol=bvol, bvol2=bvol)
        except TypeError:
            print(_euro_barrier_euro_greeks(
                char, tau, vol, k, s, 0, payoff, direc, 'C', ki, ko, lots, bvol=bvol, bvol2=bvol) is None)
...
def test_euro_barrier_pricing():
    ...
    for i in range(len(actuals)):
        ...
        val = _barrier_euro(char, tau, vol, k, s, 0,
                            payoff, direction, ki, ko, product, bvol=vol, bvol2=vol)
        try:
            assert np.isclose(val, actual)
        except AssertionError:
